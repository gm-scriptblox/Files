...
function HawkLib:Window(Win)
	if _HawkKey == "pencizurnabayilirim" then
		...
		Close.Position = UDim2.new(0, 12, 0.5, -7)
		...
		Close.MouseEnter:Connect(function()
			TweenService:Create(Close, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(220, 60, 55)}):Play()
		end)
		Close.MouseLeave:Connect(function()
			TweenService:Create(Close, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(255, 95, 87)}):Play()
		end)
		...
		-- macOS-style yellow minimize circle
		...
		Minimize.Position = UDim2.new(0, 32, 0.5, -7)
		...
		Minimize.MouseEnter:Connect(function()
			TweenService:Create(Minimize, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(220, 155, 20)}):Play()
		end)
		Minimize.MouseLeave:Connect(function()
			TweenService:Create(Minimize, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(255, 189, 46)}):Play()
		end)
		...
		function Sayfalar:Minimize(tebel)
			...
			if visibility ~= nil then
				if visibility == true then
					-- Fade the yellow circle in smoothly
					Minimize.BackgroundTransparency = 1
					Minimize.Visible = true
					TweenService:Create(Minimize, TweenInfo.new(0.3), {BackgroundTransparency = 0}):Play()

					-- Collapse/expand state
					local isMinimized = false
					local fullSize = Main.Size
					local collapsedSize = UDim2.new(Main.Size.X.Scale, Main.Size.X.Offset, 0, TitleBar.Size.Y.Offset)

					Minimize.MouseButton1Click:Connect(function()
						if not isMinimized then
							-- Collapse: hide content, shrink to title bar
							isMinimized = true
							Tabs.Visible = false
							Pages.Visible = false
							...
							-- Fade yellow button to signal it's collapsed
							TweenService:Create(Minimize, TweenInfo.new(0.2), {BackgroundTransparency = 0.3}):Play()
						else
							-- Expand: restore content and size
							isMinimized = false
							TweenService:Create(
								Main,
								TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
							...
							Pages.Visible = true
							Line_2.Visible = true
							TweenService:Create(Minimize, TweenInfo.new(0.2), {BackgroundTransparency = 0}):Play()
						end
		...
		function Sayfalar:Close(tebel)
			...
			if visibility ~= nil then
				if visibility == true then
					-- Fade the red circle in smoothly
					Close.BackgroundTransparency = 1
					Close.Visible = true
					TweenService:Create(Close, TweenInfo.new(0.3), {BackgroundTransparency = 0}):Play()
				else
					Close:Destroy()
				end
			else
			...
			-- "Are you sure?" popup builder
			local function ShowCloseConfirm(yesCallback)
				-- Overlay to block input behind popup
				...
				Popup.Name = "CloseConfirm"
				Popup.Parent = Hawk
				Popup.BackgroundColor3 = HawkLib.Themes[Theme].Main
				Popup.BackgroundTransparency = 0
				Popup.Size = UDim2.new(0, 300, 0, 140)
				Popup.Position = UDim2.new(0.5, -150, 0.5, -70)
				Popup.ZIndex = 101
				...
				PopupTitle.Text = "Close UI?"
				PopupTitle.TextColor3 = HawkLib.Themes[Theme].TitleTextColor
				...
				PopupDesc.BorderSizePixel = 0
				PopupDesc.Position = UDim2.new(0, 0, 0, 46)
				PopupDesc.Size = UDim2.new(1, 0, 0, 22)
				PopupDesc.Font = Enum.Font.Gotham
				PopupDesc.Text = "Are you sure you want to close?"
				PopupDesc.TextColor3 = HawkLib.Themes[Theme].ItemTextColors
				...
				YesBtn.Name = "Yes"
				YesBtn.Parent = Popup
				YesBtn.BackgroundColor3 = Color3.fromRGB(200, 55, 50)
				YesBtn.BorderSizePixel = 0
				YesBtn.Position = UDim2.new(0.5, 8, 0, 92)
				YesBtn.Size = UDim2.new(0, 118, 0, 34)
				...
				YesBtn.ZIndex = 102
				YesBtnCorner.CornerRadius = UDim.new(0, 7)
				YesBtnCorner.Parent = YesBtn

				-- No button (neutral)
				NoBtn.Name = "No"
				NoBtn.Parent = Popup
				NoBtn.BackgroundColor3 = HawkLib.Themes[Theme].ItemColors
				NoBtn.BorderSizePixel = 0
				NoBtn.Position = UDim2.new(0, 16, 0, 92)
				NoBtn.Size = UDim2.new(0, 118, 0, 34)
				NoBtn.Font = Enum.Font.GothamBold
				NoBtn.Text = "Cancel"
				NoBtn.TextColor3 = HawkLib.Themes[Theme].ItemTitleColors
				NoBtn.TextSize = 14
				NoBtn.AutoButtonColor = false
				NoBtn.ZIndex = 102
				NoBtnCorner.CornerRadius = UDim.new(0, 7)
				NoBtnCorner.Parent = NoBtn

				-- Hover effects
				YesBtn.MouseEnter:Connect(function()
					TweenService:Create(YesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(230, 70, 65)}):Play()
				end)
				YesBtn.MouseLeave:Connect(function()
					TweenService:Create(YesBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(200, 55, 50)}):Play()
				end)
				NoBtn.MouseEnter:Connect(function()
					TweenService:Create(NoBtn, TweenInfo.new(0.15), {BackgroundColor3 = HawkLib.Themes[Theme].Hover}):Play()
				end)
				NoBtn.MouseLeave:Connect(function()
					TweenService:Create(NoBtn, TweenInfo.new(0.15), {BackgroundColor3 = HawkLib.Themes[Theme].ItemColors}):Play()
				end)

				local function dismissPopup()
					TweenService:Create(Popup, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {BackgroundTransparency = 1}):Play()
					for _, v in ipairs(Popup:GetDescendants()) do
				...
				for _, v in ipairs(Popup:GetDescendants()) do
					...
					if v:IsA("TextButton") then v.BackgroundTransparency = 1; v.TextTransparency = 1 end
				end
				...
				for _, v in ipairs(Popup:GetDescendants()) do
					if v:IsA("TextLabel") then
						TweenService:Create(v, TweenInfo.new(0.15), {TextTransparency = 0}):Play()
					elseif v:IsA("TextButton") then
						TweenService:Create(v, TweenInfo.new(0.15), {BackgroundTransparency = 0, TextTransparency = 0}):Play()
					end
				end
			end
